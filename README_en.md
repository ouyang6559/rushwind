...
Compared to the Go predecessor [go-wind](https://github.com/tx7do/go-wind) (the same philosophy expressed in Go): RushWind is not a port but a re-expression under Rust's ownership, cancellation and error models. The semantic divergence list lives in [docs/architecture.md](./docs/architecture.md) (Chinese).
...
| Phase | Scope |
|:---|:---|
| P0 | core lifecycle, transport contracts, conformance suite ← current |
| P1 | `rushwind-transport-axum` (admin/API), `rushwind-transport-ws` (session middleware chain) |
| P2 | `rushwind-transport-quic` (QUIC/http3/webtransport), `rushwind-transport-mqtt` (external-broker consumer bridge), registration-only registry slice |
