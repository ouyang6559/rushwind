...
| script | `rushwind-script` (contract: the `ScriptEngine` lifecycle core plus a family of standalone capability traits with probe methods standing in for the Go `As*` assertions, aggregated by the blanket `FullEngine`; the `ScriptValue` data bridge; the name-keyed factory registry, the fixed and the auto-growing engine pool, and the `Manager`; plus the local source framework — memory, file with mtime-poll watching, static tree with prefix joining, dual-strategy multi-source aggregation, a TTL'd cache with watch-driven invalidation, and transform chains); counterpart of `go-scripts` |
