...
| P2 | `rushwind-transport-quic` (raw QUIC sessions + full session chain: gates/refuse, atomic admission, handshake deadline — delivered); `rushwind-transport-mqtt` (external-broker consumer bridge with per-subscription handler registration and spec-compliant wildcard dispatch — delivered); the registration-only registry slice — delivered; plus the later additions `rushwind-transport-webtransport` (wtransport session layer + full session chain) and `rushwind-transport-h3` (h3/h3-quinn request layer with request-time gates and per-connection admission) |
...
| next | kcp (if legacy interop), rushwind-protocols standalone repo |
