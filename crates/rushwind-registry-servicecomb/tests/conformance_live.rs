...
/// Service-center keys instances by their declared id: each test gets
/// its own id and port so parallel tests hold distinct instances.
fn sample(id: &str, port: u16) -> Registration {
...
async fn registered_instance_round_trips_then_leaves() {
    ...
    let registration = sample("order-01", 9000);
...
async fn watcher_delivers_addition_then_removal() {
    let registry = ServicecombRegistry::connect(&endpoint()).expect("service-center connects");
    let registration = sample("order-watch-01", 9001);
    let _handle = registry
        .register(registration.clone())
        .await
        .expect("register must succeed");
    ...
    let second = sample("order-watch-02", 9002);
    ...
    for _ in 0..30 {
        ...
        if snapshot.iter().any(|instance| {
            instance.id == "order-watch-02"
                && instance.endpoints == second.instance.endpoints
        }) {
            delivered = true;
            break;
        }
